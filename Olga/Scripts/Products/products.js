...
$(document).ready( function (event) {
	...
	$("#dZUpload").dropzone({
        url: "/Product/SaveUploadedFile?productId=" + $('#Id').val(),
        uploadMultiple: true,
		maxFiles: 10,
		...
		init: function () {
			...
			$('#savePhotoButton').off('click').click(function (clickEvent) {
				var i = 0;
				
				if (images.length) {
                    var imageString = $('#DocumentImagesListString').val();
					...
					$('#DocumentImagesListString').val(imageString);
				}
				fancybox.removeAllFiles();
			});
			$('#deletePhoto').off('click').click(function (clickEvent) {
				removeImages();
				fancybox.removeAllFiles();
			});
			$('#cancelAddPhoto').off('click').click(function (clickEvent) {
				images = [];
				fancybox.removeAllFiles();
			});
